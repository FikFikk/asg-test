            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <div class="bg-green-50 p-4 rounded-lg text-center">
                    <div class="text-2xl font-bold text-green-600">
                        {{ $routes->where('methods', 'like', '%GET%')->count() }}</div>
                    <div class="text-green-800">GET Routes</div>
                </div>
                <div class="bg-yellow-50 p-4 rounded-lg text-center">
                    <div class="text-2xl font-bold text-yellow-600">
                        {{ $routes->where('methods', 'like', '%POST%')->count() }}</div>
        ...
        <div class="bg-white rounded-lg shadow-lg p-6 mb-6">
            <div class="flex flex-wrap gap-4">
                ...
                <button onclick="clearFilters()"
                    class="px-4 py-2 bg-gray-500 text-white rounded-lg hover:bg-gray-600">Clear</button>
            </div>
        </div>
                            ...
                            <tr class="route-row">
                                <td class="px-6 py-4 whitespace-nowrap">
                                    ...
                                    @foreach ($methods as $method)
                                        @if (trim($method) !== 'HEAD')
                                            <span
                                                class="inline-block px-2 py-1 text-xs font-semibold rounded-full {{ $methodColors[trim($method)] ?? 'bg-gray-100 text-gray-800' }} mr-1 mb-1">
                                                {{ trim($method) }}
                                            </span>
                                        @endif
                                    @endforeach
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    @if ($route['name'])
                                        <span
                                            class="inline-block px-2 py-1 text-xs font-semibold bg-indigo-100 text-indigo-800 rounded-full">
                                            {{ $route['name'] }}
                                        </span>
                                    @else
                                    ...
                                    @if ($route['middleware'])
                                        <div class="text-xs text-gray-600">
                                            @foreach (explode(', ', $route['middleware']) as $middleware)
                                                <span
                                                    class="inline-block bg-gray-100 text-gray-700 px-2 py-1 rounded mr-1 mb-1">{{ $middleware }}</span>
                                            @endforeach
                                        </div>
                                    @else
                                        <span class="text-gray-400">-</span>
