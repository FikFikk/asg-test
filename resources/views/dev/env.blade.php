    ...
    <div class="container mx-auto px-4 py-8">
        ...
        <div class="bg-yellow-100 border border-yellow-400 text-yellow-700 px-4 py-3 rounded mb-6">
            <div class="flex items-center">
                ...
                <div>
                    <strong>Security Notice:</strong> Sensitive data like passwords and API keys are hidden for security
                    reasons.
                    ...
                    @foreach ($envVars as $key => $value)
                        @if (in_array($key, ['APP_NAME', 'APP_ENV', 'APP_DEBUG', 'APP_URL']))
                            <div class="flex justify-between items-center border-b border-gray-200 pb-2">
                                ...
                                <span
                                    class="text-blue-600 font-mono bg-blue-50 px-2 py-1 rounded">{{ $value ?: 'Not Set' }}</span>
                            </div>
                        @endif
                    @endforeach
                </div>
            ...
            <div class="bg-white rounded-lg shadow-lg p-6">
                ...
                <div class="space-y-3">
                    <div class="flex justify-between items-center border-b border-gray-200 pb-2">
                        ...
                        <span
                            class="text-purple-600 font-mono bg-purple-50 px-2 py-1 rounded">{{ $envVars['DB_CONNECTION'] ?: 'Not Set' }}</span>
                    </div>
                    <div class="flex justify-between items-center border-b border-gray-200 pb-2">
                        <span class="font-medium text-gray-700">DB_HOST:</span>
                        <span
                            class="text-purple-600 font-mono bg-purple-50 px-2 py-1 rounded">{{ env('DB_HOST') ?: 'Not Set' }}</span>
                    </div>
                    <div class="flex justify-between items-center border-b border-gray-200 pb-2">
                        <span class="font-medium text-gray-700">DB_PORT:</span>
                        <span
                            class="text-purple-600 font-mono bg-purple-50 px-2 py-1 rounded">{{ env('DB_PORT') ?: 'Not Set' }}</span>
                    </div>
                    <div class="flex justify-between items-center border-b border-gray-200 pb-2">
                        <span class="font-medium text-gray-700">DB_DATABASE:</span>
                        <span
                            class="text-purple-600 font-mono bg-purple-50 px-2 py-1 rounded">{{ env('DB_DATABASE') ?: 'Not Set' }}</span>
                    </div>
                    <div class="flex justify-between items-center border-b border-gray-200 pb-2">
                        <span class="font-medium text-gray-700">DB_USERNAME:</span>
                        <span
                            class="text-gray-500 font-mono bg-gray-50 px-2 py-1 rounded">{{ env('DB_USERNAME') ? '***HIDDEN***' : 'Not Set' }}</span>
                    </div>
                </div>
            </div>
                    ...
                    @foreach ($envVars as $key => $value)
                        @if (in_array($key, ['CACHE_STORE', 'SESSION_DRIVER', 'QUEUE_CONNECTION']))
                            <div class="flex justify-between items-center border-b border-gray-200 pb-2">
                                <span class="font-medium text-gray-700">{{ $key }}:</span>
                                <span
                                    class="text-orange-600 font-mono bg-orange-50 px-2 py-1 rounded">{{ $value ?: 'Not Set' }}</span>
                            </div>
                        @endif
                    @endforeach
                    <div class="flex justify-between items-center border-b border-gray-200 pb-2">
                        ...
                        <span
                            class="text-orange-600 font-mono bg-orange-50 px-2 py-1 rounded">{{ env('FILESYSTEM_DISK') ?: 'Not Set' }}</span>
                    </div>
                </div>
            </div>
                ...
                <div class="space-y-3">
                    @if ($envVars['INSTANCE_NAME'])
                        <div class="flex justify-between items-center border-b border-gray-200 pb-2">
                            ...
                            <span
                                class="text-green-600 font-mono bg-green-50 px-2 py-1 rounded">{{ $envVars['INSTANCE_NAME'] }}</span>
                        </div>
                    @endif
                    <div class="flex justify-between items-center border-b border-gray-200 pb-2">
                        <span class="font-medium text-gray-700">AWS_INSTANCE_ID:</span>
                        <span
                            class="text-green-600 font-mono bg-green-50 px-2 py-1 rounded">{{ env('AWS_INSTANCE_ID') ?: 'Not Set' }}</span>
                    </div>
                    <div class="flex justify-between items-center border-b border-gray-200 pb-2">
                        <span class="font-medium text-gray-700">AWS_AVAILABILITY_ZONE:</span>
                        <span
                            class="text-green-600 font-mono bg-green-50 px-2 py-1 rounded">{{ env('AWS_AVAILABILITY_ZONE') ?: 'Not Set' }}</span>
                    </div>
                    <div class="flex justify-between items-center border-b border-gray-200 pb-2">
                        <span class="font-medium text-gray-700">AWS_INSTANCE_TYPE:</span>
                        <span
                            class="text-green-600 font-mono bg-green-50 px-2 py-1 rounded">{{ env('AWS_INSTANCE_TYPE') ?: 'Not Set' }}</span>
                    </div>
                </div>
            </div>
        </div>
                ...
                <div class="space-y-2">
                    ...
                    <div class="text-sm space-y-1">
                        <div class="flex justify-between">
                            <span>Driver:</span>
                            <span
                                class="font-mono bg-gray-100 px-2 py-1 rounded">{{ env('MAIL_MAILER') ?: 'Not Set' }}</span>
                        </div>
                        <div class="flex justify-between">
                            <span>Host:</span>
                            <span
                                class="font-mono bg-gray-100 px-2 py-1 rounded">{{ env('MAIL_HOST') ?: 'Not Set' }}</span>
                        </div>
                        <div class="flex justify-between">
                            <span>Port:</span>
                            <span
                                class="font-mono bg-gray-100 px-2 py-1 rounded">{{ env('MAIL_PORT') ?: 'Not Set' }}</span>
                        </div>
                    </div>
                </div>
                ...
                <div class="space-y-2">
                    ...
                    <div class="text-sm space-y-1">
                        <div class="flex justify-between">
                            <span>Channel:</span>
                            <span
                                class="font-mono bg-gray-100 px-2 py-1 rounded">{{ env('LOG_CHANNEL') ?: 'Not Set' }}</span>
                        </div>
                        <div class="flex justify-between">
                            <span>Level:</span>
                            <span
                                class="font-mono bg-gray-100 px-2 py-1 rounded">{{ env('LOG_LEVEL') ?: 'Not Set' }}</span>
                        </div>
                    </div>
                </div>
                ...
                <div class="space-y-2">
                    ...
                    <div class="text-sm space-y-1">
                        <div class="flex justify-between">
                            <span>Driver:</span>
                            <span
                                class="font-mono bg-gray-100 px-2 py-1 rounded">{{ env('BROADCAST_DRIVER') ?: 'Not Set' }}</span>
                        </div>
                        <div class="flex justify-between">
                            <span>Connection:</span>
                            <span
                                class="font-mono bg-gray-100 px-2 py-1 rounded">{{ env('BROADCAST_CONNECTION') ?: 'Not Set' }}</span>
                        </div>
                    </div>
                </div>
            </div>
